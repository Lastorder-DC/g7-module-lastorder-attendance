...
- **사용자 출석 캘린더 ForEach 컴포넌트 렌더링 오류 수정**
  - g7 템플릿 엔진에 존재하지 않는 `ForEach` (type: basic) 컴포넌트 사용을 g7 표준 `iteration` 속성으로 변경
  - 캘린더 일별 출석 상태(출석/결석/미출석)가 정상적으로 표시됨

- **오늘 출석 목록이 항상 "출석 기록이 없습니다"로 표시되는 오류 수정**
  - `today()` 컨트롤러 메소드에서 `successWithResource()`를 `success()`로 변경
  - `successWithResource()`는 컬렉션에 `resolve()`를 호출하여 페이지네이션 메타데이터가 소실되고, 응답 구조가 `data.data` 대신 단순 배열로 반환되어 프론트엔드에서 데이터를 찾지 못하는 문제 해결
  - 이 수정으로 출석점수 0점 표시, 출석시간 미표시 문제도 함께 해결됨

- **빈 출석 목록 메시지 번역키 수정**
  - 사용자 페이지에서 관리자용 번역키(`admin.attendance.no_data`)를 사용하던 것을 사용자용 번역키(`user.table.no_data`)로 변경

- **현재 시간 표시 실시간 갱신 수정**
  - 페이지 상단 현재 시간이 페이지 로드 시점에만 표시되고 갱신되지 않던 문제 수정
  - `loadScript` init 액션을 통해 1초마다 DOM을 갱신하는 경량 스크립트 주입

- **개근점수 자세히 보기 기능 추가**
  - 클릭 시 연속출석 보너스 기준(7일/30일/365일) 및 포인트가 표시되는 확장 패널 추가

- **랭킹점수 자세히 보기 기능 추가**
  - 클릭 시 순위별 보너스 포인트 기준(1등/2등/3등)이 표시되는 확장 패널 추가

- **개근분류 자세히 보기 기능 추가**
  - 클릭 시 개근 분류(주간/월간/연간)와 각 분류의 표시 색상이 표시되는 확장 패널 추가
